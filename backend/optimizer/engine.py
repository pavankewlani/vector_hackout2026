def dispatch_energy(demand, solar_available, wind_available, battery_percent, battery_minimum, diesel_price, forecast_solar=0, forecast_wind=0, forecast_demand=None, battery_capacity_kw=None):
    ...
    recommendation = "Charge battery now before the forecast renewable dip." if forecast_change < 0 and battery_percent > battery_minimum else ("Hold diesel in reserve while renewables meet demand." if diesel == 0 else "Diesel support is required to protect reliability at current demand.")
